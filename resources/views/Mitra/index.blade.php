    ...
    <form onsubmit="return false" class="w-full max-w-10xl relative group mb-8">
        ...
        <div class="relative flex items-center bg-[#151515] border border-white/10 rounded-full pl-6 pr-2 py-2">
            <input
                type="text"
                id="searchInput"
                placeholder="Cari job, type, jenjang, keahlian, atau keterangan..."
                ...
                @forelse($mitras as $m)
                    <tr class="mitra-row">
                        ...
                        <td class="text-center">
                            @if($m->mitra?->foto)
                                <img src="{{ Storage::url($m->mitra?->foto) }}"
        ...
        @forelse($mitras as $m)
            <div class="glass p-4 rounded-xl mitra-card">
                <div class="flex items-center gap-4">
                    @if($m->mitra?->foto)
                        <img src="{{ Storage::url($m->mitra?->foto) }}" class="w-12 h-12 rounded-full object-cover">
                    @else
